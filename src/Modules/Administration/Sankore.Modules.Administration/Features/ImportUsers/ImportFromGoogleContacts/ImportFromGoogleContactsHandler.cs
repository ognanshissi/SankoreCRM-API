...
internal sealed class ImportFromGoogleContactsHandler(
    ...
    TimeProvider clock)
    : IRequestHandler<ImportFromGoogleContactsCommand, Result<Guid>>
{
    public async Task<Result<Guid>> Handle(ImportFromGoogleContactsCommand cmd, CancellationToken ct)
    {
