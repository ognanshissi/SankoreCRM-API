    ...
    : IRequestHandler<ActivateTemplateCommand, Result>
{
    public async Task<Result> Handle(ActivateTemplateCommand request, CancellationToken ct)
    {
        ...
        // Replace only auto-generated transitions (preserves custom conditional ones).
        var oldAutoTransitions = await db.WorkflowTransitions
            .Where(t => t.TemplateId == template.Id && t.IsAutoGenerated)
            .ToListAsync(ct);
        db.WorkflowTransitions.RemoveRange(oldAutoTransitions);
