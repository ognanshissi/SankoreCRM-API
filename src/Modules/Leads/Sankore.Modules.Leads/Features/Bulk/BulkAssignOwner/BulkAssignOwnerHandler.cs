...
internal sealed class BulkAssignOwnerHandler(LeadsDbContext db)
    : IRequestHandler<BulkAssignOwnerCommand, Result<BulkOperationResult>>
{
    public async Task<Result<BulkOperationResult>> Handle(
        BulkAssignOwnerCommand cmd, CancellationToken ct)
    {
